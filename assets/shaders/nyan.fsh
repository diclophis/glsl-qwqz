...
vec3 materialBaseColor( float t ) {
  ...
  //return (vec3(t) * vec3(0.14591255443,0.34934560423, t)).xyz;
  return vec3(0.61, 0.61, 0.61);
}
...
vec3 getColor(vec2 coord) {
  ...
  // nyan cat! at z=-7;
  z = 7.; vec2 dxy = vec2( z );
  vec2 nyanpos = coord*dxy+vec2( 6.5, 1.2 + (sin(iGlobalTime) * 10.0));
  bool nyanhit = false;
  ...
  if( !nyanhit ) {
    float ao = length(position- vec3( -3.5, 1.3 + (sin(iGlobalTime * 2.0)), 6.));
...
void main(void) {
  ...
  color = pow( 0.7*color, vec3(EXPOSURE) ); 
    // vigneting
    color *= 0.25+0.75*pow( 16.0*q.x*q.y*(1.0-q.x)*(1.0-q.y), 0.5 );
